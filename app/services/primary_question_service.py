...
from app.services.ai_agents.Interview.primary_question_generator.agent import (
    primary_question_generator_agent,
    primary_question_generator_runner,
)
...
async def generate_primary_question(
    ...
    """
    Generate the primary interview question using PrimaryQuestionGeneratorAgent.
    ...
    input_data = PrimaryQuestionInput(
    ...
    try:
        result = await run_agent_with_runner(
            runner=primary_question_generator_runner,
            agent=primary_question_generator_agent,
            prompt=prompt,
        )
        # Validate output using Pydantic model
        if isinstance(result, dict):
            output = PrimaryQuestionOutput(**result)
        else:
            # If result is a string, try to parse it as JSON
            try:
                parsed = json.loads(result) if isinstance(result, str) else result
                output = PrimaryQuestionOutput(**parsed)
        ...
        logger.error(f"Error generating primary question: {e}")
        raise
