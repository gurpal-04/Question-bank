...
class PrimaryQuestionInput(BaseModel):
    """
    Input schema for PrimaryQuestionGeneratorAgent.
    This represents the input required to generate the primary interview question.
    """
...
class PrimaryQuestionOutput(BaseModel):
    ...
    This represents the primary interview question generated by the agent.
    """
...
class FollowUpQuestionInput(BaseModel):
    """
    Input schema for FollowUpQuestionGeneratorAgent.
    """

    interview_context: Dict[str, Any] = Field(
        ..., description="Context for tone calibration"
    )
    primary_skill: str = Field(..., description="The primary skill being tested")
    previous_question: str = Field(..., description="The last question asked")
    candidate_answer: str = Field(..., description="The candidate's answer")
    evaluation_signals: Dict[str, Any] = Field(..., description="Gap analysis output")
    followup_intent: str = Field(..., description="Selected intent for follow-up")


class FollowUpQuestionOutput(BaseModel):
    """
    Output schema for FollowUpQuestionGeneratorAgent.
    """
